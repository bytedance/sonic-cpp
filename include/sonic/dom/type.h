...
enum TypeFlag {
  ...
  // kStringCopy: sv.p is copied, but not need free, e.g. node's string buffer is dom str_
  kStringCopy = kString,                      // xxx00_100, 4
  // kStringFree: sv.p is copied and need free, e.g. SetString with allocator arg
  kStringFree = ((uint8_t)(1 << 3)) | kString,  // xxx01_100, 12
  // kStringConst: sv.p is not copied, so not need free, e.g. SetString without allocator arg
