    ...
    for (auto itr = patch.MemberBegin(), e = patch.MemberEnd(); itr != e;
      ...
      if (itr->value.IsNull()) {
        ...
        if (m == original.MemberEnd()) {
          T null_node{};
          auto n =
              original.AddMember(itr->name.GetStringView(), std::move(null_node), alloc);
    ...
    if (!original.IsObject()) {
      ...
      for (auto itr = src.MemberBegin(), e = src.MemberEnd(); itr != e; ++itr) {
        ...
        if (m == original.MemberEnd()) {
          ...
          original.AddMember(itr->name.GetStringView(), std::move(new_node), alloc);
        } else {
    ...
    case kObject:
      ...
      for (auto it = from.MemberBegin(), e = from.MemberEnd(); it != e; ++it) {
        ...
        if (m != to.MemberEnd()) {
          ...
          if (b) {
            is_diff = true;
            patch.AddMember(it->name.GetStringView(), std::move(tmp), alloc);
          }
