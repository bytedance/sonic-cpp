...
struct GetUintHandler {
    ...
    bool Key(const char* key, SizeType length, bool copy) {
        ...
        // match the key
        if (length == target.size() && std::memcmp(target.data(), key, length) == 0) {
            level_++;
        }
        return true;
    }
...
static bool RapidjsonSaxOnDemand(const std::string& json, const std::vector<std::string_view>& path, uint64_t& val) {
