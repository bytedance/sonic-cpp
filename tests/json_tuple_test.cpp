...
TEST(JsonTuple, EscapedKeys) {
  // Keys contain '\\' and '\"' in JSON source.
  std::string json = R"({"a\\": 1, "b\"": 2})";
  std::vector<std::string_view> paths{"a\\", "b\""};
  std::vector<std::optional<std::string>> expected = {"1", "2"};
...
TEST(JsonTuple, NoMatchAllKeys) {
  ...
  std::vector<std::string_view> paths{"x", "y"};
  std::vector<std::optional<std::string>> expected = {std::nullopt,
                                                      std::nullopt};
